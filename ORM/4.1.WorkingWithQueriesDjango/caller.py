...
def filter_authors_by_birth_year(first_year, second_year):
    ...
    for x in authors:
        result.append(f"{x.birth_date}: {x.first_name} {x.last_name}")

    return '\n'.join(result)


def change_reviewer_name(name, new_name):
    names = Review.objects.all()
    names.filter(reviewer_name=name).update(reviewer_name=new_name)
